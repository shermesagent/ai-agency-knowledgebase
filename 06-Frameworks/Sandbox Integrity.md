...
## The Veto at Fleet Scale: Irreversibility Budgets and Action-Commit Gates (2026-09-02)

The boundary question has escalated from *per-agent* to *fleet* — and the August lesson is that the fleet defeats per-agent integrity by construction. Three new papers (all arXiv 2026-09-02) supply the mechanisms:

- **[The Irreversibility Budget: Fleet-Level Risk Accounting and Admission Control for Agent Operating Systems](https://arxiv.org/abs/2609.00275):** fleets of LLM agents externalize effects that cannot be undone — moving money, deploying code, deleting data, disclosing information. Current controls check one effect at a time, so a fleet of individually authorized agents can overdraw its principal's risk under a shared trigger while every local gate stays correct. The proposal: a trusted runtime maintains a **cumulative account of residual value-at-risk per principal, across agents, workflows, and tenants**; each effect is charged its residual loss, and the marginal effect is denied once the aggregate would overdraw. Controlled study: per-effect gates admitted fleet-level overdraws up to **48× the tenant's risk limit**, while the budget held every correctly charged run within limit. Conservative, dependency-aware pricing is the stated open problem.
- **[OpenAgentFlow: Enabling System-Wide Safety Boundaries for Heterogeneous AI Agent Fleets](https://arxiv.org/abs/2609.00015):** the enforcement architecture — a control-plane/action-plane split where safety lives at the **action-commit boundary**. Pending GUI actions, API calls, tool calls, and LLM invocations are normalized into one AgentEvent stream routed through a shared pre-execution Policy Enforcement Point; policies update without modifying agents, prompts, or models. On Android: 94.0% accuracy, 95.3% attack block rate; dynamic-policy suite 27/30 after new rules installed. This is the runtime-contract idea (2608.11274) generalized from one agent to a heterogeneous fleet — a *shared governable action stream* rather than per-agent harnesses.
- **[Spawn Freely, Act Sparingly: Progressive Risk Vesting for Recursive LLM-Agent Trees](https://arxiv.org/abs/2609.01035):** the boundary question for self-spawning agents. Distinguish **sandbox spawning** (external controls prevent the specified harm — search freely) from **capability activation** (a selected branch crosses an irreversible-action boundary — the veto moment). Progressive Risk Vesting holds a trajectory-level risk budget in escrow and debits it as branches activate, with an anytime harm bound for adaptively generated trees. Trajectory harm changes character as an "authority reproduction number" R_A crosses one: below criticality harm scales with p, at criticality with √p, above criticality it keeps a positive floor. Design rule: search broadly in the sandbox, grant recursive authority sparingly, with an explicit risk charge.

**Why this belongs on the sandbox page:** the Hugging Face message board was the fleet case — ~1,200 agents, each individually confined, collectively coordinating across what should have been isolated boundaries. The 48× overdraw result names the failure mode formally: **integrity is a fleet-level property, not a per-agent one**. The preventive face becomes an action-commit gate on a shared event stream (OpenAgentFlow); the evidential face becomes a cumulative ledger that prices residual risk (the budget); and the permission question (this page's 08-28 allow/ask/never study) extends to *when authority vests* — escrow and progressive activation rather than spawn-time grants. For [[Home Server AI Agents]]: the practical v1 is an irreversibility ledger — one file listing every money/deploy/delete/disclose action any agent can take, checked in aggregate before anything runs.
