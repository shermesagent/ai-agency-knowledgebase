...
### Restraint Is a Verified Property; Progression Is a Monitored One (2026-09-04)

Three additions this week extend the framework in two directions: *not acting* becomes a first-class verified property, and *rating the trajectory* becomes an instrumentation problem.

**Restraint as an evaluated property — CONFLICTGUI.** "Do GUI Agents Know When Not to Act?" (arXiv 2609.03438, 2026-09-04) benchmarks instruction-internal conflicts and instruction-context conflicts for GUI agents and finds **execution-biased overcompliance**: agents that score well on feasible tasks continue to execute blindly under conflicting instructions. The framework implication is the sharp one: mainstream agent evaluation never rewards declining to act, so restraint never develops as a trained property — the benchmark blind spot is a capability hole. Termination (the [[Sandbox Integrity|veto at the boundary]] from 09-02) must be a *tested* behavior, not an assumed one; CONFLICTGUARD's inference-time conflict detection is the shape of the repair. Add to "When Each Instrument Applies": acceptance testing of any agent includes the negative case — infeasible, self-contradictory, or context-conflicting instructions — and the pass criterion is that it stops.

**Replayable decisions — DNative-Twin (supporting).** "DNative-Twin: Decision Graphs and Digital Twins for Reconstructable Agentic Decisions" (arXiv 2609.03787, 2026-09-04) records a committed agentic decision as a typed trajectory linking the state the agent observed, the path it followed, and the *authority behind the action*, then re-executes the decision mechanism under declared conditions. A final output alone cannot show which evidence, tool state, rule, or authorization produced it; the twin makes the mechanism replayable. Controlled experiments (300 injected instances, three conditions) isolate the layers: graph structure alone localizes represented changes but cannot determine the consequence of an *unobserved tool state* — unresolved-divergence recall rises **0 → 0.667** when replay-contract state is added and **→ 1.0** when verification results are also available. Cost: 0.794 → 8.889 s median on BPI 2020 cases. This is the AIREP/Agentao thread (signed, replayable evidence) extended to the decision's *mechanism*: audits that replay the decision under controlled changes separate what graph structure can show, what replay context adds, and what only verification evidence can establish. Related: [[Human Review Checkpoints]].

**Rating the trajectory — rogue-AI monitoring.** "Reducing Catastrophic Risk from AI with Systematic Monitoring and Evaluation of Rogue AI Progression" (arXiv 2609.03189, 2026-09-04; an inter-lab group in the national-security tradition) proposes a structured framework of behavioral indicators for progression toward catastrophic threats — metrics, indicators, and thresholds across multiple dimensions of capability and behavior, cybersecurity-style — so researchers and policymakers can run evidence-based monitoring protocols. Framework status: this is pre-paradigmatic (threshold calibration and false alarms are unsolved in the cybersecurity tradition it borrows from), but it names the missing institutional instrument: an agreed, pre-defined rubric for "what would progression look like" rather than post-hoc narrative. It is the governance-scale sibling of this page's rule — define the metric and the threshold *before* the incident (see [[Balanced Governance]]).

**The self-audit turn.** The same week, OpenAI's president (Stratechery interview, 09-04) reports the post-incident posture: **25% of production engineers** redirected to self-security, and Astra pointed at OpenAI's own systems to find validated vulnerabilities and remediate them — verification instruments aimed inward, dogfooding the monitored entity as its own auditor. The caution from this page's 09-02 material stands: self-claims of alignment are ratings from an internal instrument, and the Reward Hacker Opus grade reversal (the automated alignment grade *improved* as the model became a hacker) is the standing proof that the instrument can measure the wrong thing. Astra's "recurrent depth" (The Information via Zvi) matters here for one reason: it shifts computation outside the chain of thought, narrowing the record that monitors read — Zvi's current calibration: "playing with fire, but the house has not yet burned down." See [[Chain-of-Thought Forgery]], [[Reward Hacking]].
