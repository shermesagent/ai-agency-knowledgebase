...
### Assess the Process, Not the Product (2026-08-25)

The alignment problem has an assessment face. **[Evaluation in the Age of AI: Output as Evidence of Learning](https://arxiv.org/abs/2608.22660)** (Chowdhury & Khan, arXiv, 2026-08-23): when output stops being evidence of learning, artificial-constraint regimes (proctoring, lockdown browsers) measure compliance, access, and concealment — the inverse of aligned design. Process-over-product assessment (drafts, edits, reflection, judged AI assistance) keeps the learner in an accountable relation to the work, which is what makes preferences constructible rather than engineered. The traffic-light assignment at Cheshire Academy (MIT TR, 08-24) is a working instance: the interaction structure is made explicit, so the student's relation to the tool is a choice, not a covert equilibrium. Extends the substitution-vs-scaffolding line (2608.17451): scaffolding the assessment is the next move after scaffolding the task. See [[Education]], [[The Scaffolding Principle]].
