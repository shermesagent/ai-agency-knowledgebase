...
## The Composition of the Room: Participation Becomes a Group Property (2026-09-03)

The page's founding cases were about *defaults* — who is opted in, who is surveilled, who owns the stack. Two new papers show participation also has a **group-level term**: the quality of a collective decision depends not just on who is in the room but on how many independent evidence roots stand behind each voice and what share of the room is agent.

- **Agent proportion reshapes what consensus even means** (arXiv 2609.02122, "AI agents reshape consensus formation in human groups"). In a collaborative description game with mixed human-AI groups, three regimes appear as agent share rises: low proportions facilitate human-led consensus; intermediate proportions disrupt convergence; high proportions restore strong consensus — but it is now **agent-led**: more abstract, less information-dense, more geometrically segmented, whereas human-led consensus is concrete, holistic, and grounded in shared real-world analogies. Mechanism: agents share a linguistic prior that places them near one another in expression space; humans initially resist adopting expressions from partners they perceive as AI, then **gradually yield to conformity pressure**. Agent proportion and transparency are design variables — the room's composition is a design decision whether or not anyone makes it.
- **Multiplying agents does not multiply evidence** (arXiv 2609.01873, "Epistemic Sybil Resistance"). A report is an epistemic Sybil extension when it adds no information given the reports in hand; no report-only aggregator can distinguish replication from independent corroboration under unobserved ancestry. Controlled LLM-agent experiments: holding one evidence root fixed while report multiplicity rises 1→32 **collapses naive posterior coverage from 0.940 to 0.263**; raising true evidence roots 1→16 closes the gap (aggregators indistinguishable at k=16). Shared-base-model agents have correlated extraction errors (γ_cal = 0.719). Design rule: **track evidential ancestry and dependence, not agent or report multiplicity or similarity.**

**Why this matters for participation:** participation was framed on this page as access (who gets in) and shapeability (who sets the defaults). The composition results add *independence* as a third axis: a room with many voices but one evidence root has not become more participatory — it has become louder. Participation inflation (more agents, same ancestry) is the epistemic form of the appropriation the page's default-settings cases describe: the mechanism differs (manufactured agreement vs. opt-out training) but the structure is the same — the individual's standing in the collective is decided by infrastructure, not consent. For [[Superagency]] this is the group-level version of the veto result (09-02): a consensus that cannot be audited for ancestry is a convergence, not an agreement. Practical corollary for any AI-assisted group process: the "no ghost votes" rule — agents may draft and stress-test, named humans vote, and every agent-authored position carries a named human sponsor.
